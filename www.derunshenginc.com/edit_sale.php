    ...
    validate_fields($req_fields);
        if(empty($errors)){
          ...
          if( $result && $db->affected_rows() === 1){
                    ...
                    $session->msg('s',"销售更新成功");
                    redirect('sales.php', false);
                  } else {
                    $session->msg('d',' 销售更新失败');
  ...
  <div class="panel">
    <div class="panel-heading clearfix">
     ...
     <div class="pull-right">
       <a href="sales.php" class="btn btn-primary">显示所有销售</a>
     </div>
    </div>
          ...
          <th> 编辑</th>
           ...
           <tbody  id="product_info">
              ...
              <form method="post" action="edit_sale.php?id=<?php echo (int)$sale['id']; ?>">
                ...
                <td>
                  <input readonly="readonly" type="text" class="form-control" name="total" value="<?php echo remove_junk($sale['price']); ?>">
                </td>
                <td id="s_date">
                  <input type="date" class="form-control datepicker" name="date" data-date-format="" value="<?php echo remove_junk($sale['date']); ?>">
                </td>
                <td>
                  <button type="submit" name="update_sale" class="btn btn-primary">更新</button>
                </td>
              </form>
              </tr>
           </tbody>
